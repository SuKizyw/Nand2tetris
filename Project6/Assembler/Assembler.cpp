...
int main() {
    std::string filepath = "Code\\Assembler\\Pong.asm";
    std::string hackfile = "Code\\Assembler\\Pong.hack";
    ...
    while (secondparser.hasMoreCommands()){
        ...
        if(command_type == secondparser.A_COMMAND){
            if(!isdigit(secondparser.symbol(secondparser.current_command)[0])){
                if(!symboltable.contains(secondparser.symbol(secondparser.current_command))){
                    symboltable.addEntry(secondparser.symbol(secondparser.current_command), ram_address++);}
                hack << std::bitset<16>(symboltable.GetAddress(secondparser.symbol(secondparser.current_command))) << std::endl;
            }
                else{hack << std::bitset<16>(std::stoi(secondparser.symbol(secondparser.current_command))) << std::endl;}
        }
        else if(command_type == secondparser.C_COMMAND){
            hack << "111" << hackCode.comp(secondparser.comp(secondparser.current_command)) << hackCode.dest(secondparser.dest(secondparser.current_command)) << hackCode.jump(secondparser.jump(secondparser.current_command)) << std::endl;
            std::cout <<secondparser.jump(secondparser.current_command)<< ":"<< hackCode.jump(secondparser.jump(secondparser.current_command)) << std::endl;
        }
